...
DeepFlow::DeepFlow() : deep_flow(createOptFlow_DeepFlow()) {}

/******************************************************************************/

void DeepFlow::compute_flow(
  const Mat& previous_frame,
  const Mat& current_frame,
  Mat& optical_flow
) {
  deep_flow->calc(previous_frame, current_frame, optical_flow);
}

/******************************************************************************/

int DeepFlow::get_frame_type() {
  return CV_8UC1;
}
